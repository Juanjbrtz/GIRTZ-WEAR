...
function getImage(formData: FormData) {
  ...
  if (!value || typeof value === "string" || value.size === 0) throw new Error("Debes seleccionar una foto.");
...
export async function createProductSafe(formData: FormData) {
  await requireAdmin();
  ...
  if (!name || price < 100) throw new Error("Completa foto, nombre y un precio válido.");
  ...
  if (existing) {
    redirect(`/admin/products/${existing.id}/edit?duplicate=1`);
  }
  ...
  try {
    ...
    if (alreadyCreated) {
      redirect(`/admin/products/${alreadyCreated.id}/edit?duplicate=1`);
    }
    throw error;
  }
}
