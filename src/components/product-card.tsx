...
export function ProductCard({
  ...
  return (
    <article className="product-card product-card-v3 product-card-v4">
      <Link href={`/product/${product.slug}`} className="product-media product-media-v3 product-media-v4">
        <ProductVisual
          product={product}
          sizes="(max-width: 720px) 50vw, (max-width: 1100px) 46vw, 25vw"
        />
      ...
      <div className="product-info product-info-v3 product-info-v4">
        ...
        <Link href={`/product/${product.slug}`} className="product-detail-link-v3 product-detail-link-v4">
          VER DETALLES DEL MODELO <span>→</span>
        </Link>
      </div>
    </article>
  );
